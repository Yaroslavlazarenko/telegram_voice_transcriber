    ...
    async def bot_callback_handler(self, event):
        ...
        if fix_data:
            try:
                ...
                # 2. Обновляем сообщение бота (Добавлен parse_mode='html')
                await event.edit(
                    f"✅ <b>Готово!</b>\nСообщение отредактировано.",
                    buttons=[Button.url("🔗 Вернуться к сообщению", fix_data["link"])],
                    parse_mode='html'  # <--- КРИТИЧЕСКИ ВАЖНО
                )
                
                del self.fix_cache[fix_id]
                logger.info(f"Сообщение {fix_data['msg_id']} успешно отредактировано.")
                
            except Exception as e:
                logger.error(f"Ошибка применения правки: {e}")
                await event.answer("Ошибка: не удалось отредактировать сообщение.", alert=True)
        else:
            await event.answer("Ошибка: данные устарели (бот был перезагружен).", alert=True)
